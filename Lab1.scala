...
object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
  ...
  def abs(n: Double): Double = if (n * (-1.0) > n) n * (-1.0) else n

  def xor(a: Boolean, b: Boolean): Boolean = if (a == b) false else true

  def repeat(s: String, n: Int): String = {
    if (n == 0) return ""
    if (n < 0) throw new IllegalArgumentException("Number of repetitions must be positive")
    return s + repeat(s, n - 1)
  }

  def sqrtStep(c: Double, xn: Double): Double = xn - ((pow(xn, 2) - c) / (2 * xn))

  def sqrtN(c: Double, x0: Double, n: Int): Double = {
    if (n == 0) return x0
    if (n < 0) throw new IllegalArgumentException("Input n must be positive")
    val out: Double = sqrtN(c, sqrtStep(c, x0), n - 1)
    return out
  }

  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
    if (epsilon <= 0) throw new IllegalArgumentException("Epsilon must be positive")
    val step: Double = sqrtStep(c, x0)
    if (abs(pow(step, 2) - c) < epsilon) step else sqrtErr(c, step, epsilon)
  }
  ...
  def insert(t: SearchTree, n: Int): SearchTree = t match {
    ...
    case Node(l, d, r) => if (n < d) Node(insert(Node(l, d, r).l, n), d, r) else Node(l, d, insert(Node(l, d, r).r, n))
  }
  ...
  def delete(t: SearchTree, n: Int): SearchTree = t match {
    ...
    case Node(l, d, r) =>
      if (n==d) { val (r1, data) = deleteMin(r)
        Node(l,data,r1)
      } else if (n < d) Node(delete(l,n),d,r) else Node(l,d,delete(r,n))
  }
  ...
  def eval(e: Expr): Double = e match {
    ...
    case _ => throw new IllegalArgumentException("Operation must be either negative, plus, minus, times, or divide")
  }
